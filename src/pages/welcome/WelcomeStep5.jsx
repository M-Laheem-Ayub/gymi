    ...
    return (
        <div className="relative h-screen w-screen overflow-hidden bg-black text-white font-sans">
            <div className="absolute inset-0 z-0">
                <img src="/images/slide5.png" alt="Background" className="h-full w-full object-cover opacity-60" />
            ...
            <div className="absolute inset-0 z-10 flex flex-col justify-between pointer-events-none">
                    ...
                    <AnimatePresence>
                        ...
                        >
                            ...
                            <p className="text-sm leading-relaxed text-gray-200">
                                From HIIT to Yoga, access hundreds of classes led by expert coaches tailored for every fitness level and goal.
                            </p>
                        </motion.div>
                    </AnimatePresence>
                </div>
                ...
                <div className="flex justify-center space-x-2 mb-8 pointer-events-auto">
                    {[1, 2, 3, 4, 5].map((step) => (
                        <div key={step} className={`h-1 rounded-full transition-all duration-300 ${step === 5 ? 'w-8 bg-white' : 'w-4 bg-gray-600'}`} />
                    ))}
                </div>
            ...
            <div className="absolute inset-y-0 left-0 w-1/3 z-20 cursor-pointer" onClick={() => navigate('/welcome/4')} />
            {/* Last Step: Right tap can loop or go to Signup? Keeping it optional or loop */}
            <div className="absolute inset-y-0 right-0 w-1/3 z-20 cursor-pointer" onClick={() => navigate('/signup')} />
        </div>
    );
};

export default WelcomeStep5;
